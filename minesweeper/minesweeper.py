...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for c in self.neighbors(cell):
            if c in self.mines:
                new_sentence_count -= 1
            elif c not in self.safes:
                new_sentence_cells.add(c)
        ...
        while True:
            new_safes = set()
            new_mines = set()
            self.update_availiable(new_mines, new_safes)
            if len(new_mines) == len(new_safes) == 0:
                break
        ...
        for s1 in self.knowledge:
            for s2 in self.knowledge:
                if s1 != s2 and s1.cells.issubset(s2.cells):
                    new.append(Sentence(s2.cells - s1.cells, s2.count - s1.count))
    ...
    def update_availiable(self, new_mines, new_safes):
        """
        Save all new known mines and safes into to `new_mines and `new_safes.
        """
        for sentence in self.knowledge:
